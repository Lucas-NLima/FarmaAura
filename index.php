...
foreach ($_SESSION['carrinho'] as $id => $qtd) {
    $produto = $produtoController->buscarProduto($id);
    if ($produto)
        $totalCarrinho += $produto['preco'] * $qtd;
}
...
<body>
    ...
    <main class="produtos">
        <?php foreach ($produtos as $produto): ?>
            <div class="produto-card">
                <img src="img/<?= htmlspecialchars($produto['imagem'] ?? 'default.png') ?>"
                    alt="<?= htmlspecialchars($produto['nome']) ?>">
                <h3><?= htmlspecialchars($produto['nome']) ?></h3>
                <p class="preco">R$ <?= number_format($produto['preco'], 2, ',', '.') ?></p>
                <div class="botoes-produto">
                    <button onclick="window.location.href='produtos_detalhes.php?id=<?= $produto['id'] ?>'">Mais
                        Informações</button>
                    <button onclick="window.location.href='index.php?acao=adicionar&id=<?= $produto['id'] ?>'">Adicionar ao
                        Carrinho</button>
                </div>
            </div>
        <?php endforeach; ?>
    ...
    <div id="popup" class="popup">
        <div class="popup-content">
            ...
            <img src="img/promos.png" alt="Promoção FarmaAura" class="popup-img">
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Verifica se o popup já foi mostrado antes
            const popupShown = localStorage.getItem("popupShown");

            if (!popupShown) {
                // Mostra o popup após 1 segundo (somente na primeira vez)
                setTimeout(() => {
                    popup.classList.add("show");
                    localStorage.setItem("popupShown", "true"); // marca que já foi exibido
                }, 1000);
            }
